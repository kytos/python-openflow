...
class TestFlowMod(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.flow_mod = flow_mod.FlowMod(xid=1, command=1, match=match,
                                         cookie=0, idle_timeout=300,
                                         hard_timeout=6000, priority=1,
                                         buffer_id=1, out_port=80, flags=0,
                                         actions=[action_header])

    def test_get_size(self):
        self.assertEqual(self.flow_mod.get_size(), 72)
